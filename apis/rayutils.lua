...
rayutils.ray = {
  __index = function(ray, key)
    if key == 'p' then return ray[1] -- get the starting point
    elseif key == 'dv' then return ray[2] -- get the unitary directional vector
    elseif key == "nv"then return vector(ray[2].y, -ray[2].x); -- get a normal vector
...
function rayutils:new_ray(...)
  ...
  --  return setmetatable({vector(m[1][1], m[1][2]), vector(m[2][1]-m[1][1], m[2][2]-m[1][2])^1 ,m[2][2], -m[2][1], -m[2][2]*m[1][1]+m[2][1]*m[1][2]}, self.ray)
