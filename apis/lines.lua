...
function lines:new_line(...)
  ...
  local m = {...}
  if asserter.assert_form(m, {{0,0},{0,0}}) then -- 2 points P, Q respectively and their coordinates
  ...
  elseif asserter.assert_form(m, {0,0,0}) then -- given a, b and c
    return setmetatable({a,b,c},self.line);
  elseif asserter.assert_form(m, {{0,0}, vector(0,0)) --Given a point and the directional vector.
...
function lines.get_intersection(line1, line2)
